...
function main(argv) {
  ...
  const pathKey = env.PATH !== undefined || process.platform !== "win32" ? "PATH" : "Path";
  env[pathKey] = `${binDir}${delimiter}${env[pathKey] ?? env.PATH ?? env.Path ?? ""}`;
  // Windows npm shims are `.cmd`; spawn without a shell cannot exec them.
